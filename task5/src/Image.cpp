...
Image::Image(const unsigned int width, const unsigned int height, ImageLoader &loader)
    : width(width), height(height), loader(loader), pixels(width * height, Pixel(0, 0, 0)) {
}

Image::Image(unsigned int width, unsigned int height, const Pixel &fill_with, ImageLoader &loader)
    : width(width), height(height), loader(loader), pixels(width * height, fill_with) {
}
...
void Image::set_loader(const ImageLoader &loader) const {
    this->loader = loader;
}
